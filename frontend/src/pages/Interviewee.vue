...
async function startInterview() {
  ...
  try {
    ...
    interviewResult.value = await res.json()
  } catch (e) {
    alert(e.message)
  } finally {
...
<template>
  <div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-2xl">
      ...
      <!-- 面试结果展示（展开后） -->
      <template v-if="interviewResult">
        <div class="text-center mb-8">
          <h1 class="text-3xl font-bold text-white mb-2">面试已就绪</h1>
          <p class="text-slate-400">JD 和简历内容已加载，可在下方查看</p>
        </div>
        ...
        <div class="mb-6">
          ...
          <pre class="w-full rounded-xl bg-slate-800/50 border border-slate-600 text-slate-300 text-sm p-5 whitespace-pre-wrap overflow-auto max-h-64">{{ interviewResult.jd }}</pre>
        </div>
        ...
        <div class="mb-8">
          ...
          <pre class="w-full rounded-xl bg-slate-800/50 border border-slate-600 text-slate-300 text-sm p-5 whitespace-pre-wrap overflow-auto max-h-64">{{ interviewResult.resume }}</pre>
        </div>
      ...
      <template v-else>
        ...
        >
          <span v-if="starting" class="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
